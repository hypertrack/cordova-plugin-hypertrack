...
@objc final class HyperTrackCordovaPlugin: CDVPlugin {
    ...
    @objc func initialize(_ command: CDVInvokedUrlCommand) {
        sendAsResult(
            initializeSDK(command.arguments[0] as! Dictionary<String, Any>).map({ SuccessResult in
                initListeners()
                return .void
            }),
            self,
    ...
    @objc func addGeotag(_ command: CDVInvokedUrlCommand) {
        sendAsResult(
            addGeotagMethod(command.arguments[0] as! Dictionary<String, Any>),
            self,
            command
        )
    }
    ...
    @objc func setMetadata(_ command: CDVInvokedUrlCommand) {
        sendAsResult(
            setMetadataMethod(command.arguments[0] as! Dictionary<String, Any>),
            self,
            command
        )
    }
    ...
    @objc func setName(_ command: CDVInvokedUrlCommand) {
        sendAsResult(
            setNameMethod(command.arguments[0] as! Dictionary<String, Any>),
            self,
            command
        )
    }
    ...
    @objc func setAvailability(_ command: CDVInvokedUrlCommand) {
        sendAsResult(
            setAvailabilityMethod(command.arguments[0] as! Dictionary<String, Any>),
            self,
            command
        )
    }
    ...
    private func sendTrackingEvent(isTracking: Bool) {
        ...
        let result = CDVPluginResult.init(
